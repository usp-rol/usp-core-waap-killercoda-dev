...
while [ $RC -gt 0 ]; do
  sleep 2
  kubectl wait pods -l app.kubernetes.io/name=usp-core-waap -n petstore --for='condition=Ready' --timeout=10s
  RC=$?
done
...
while [ $RC -gt 0 ]; do
  ...
  nohup kubectl -n petstore port-forward svc/petstore-usp-core-waap 80:8080 --address 0.0.0.0 >/dev/null &
