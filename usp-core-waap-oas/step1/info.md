...
**intially the petstore API will be access unprotected via localhost:8080 (port 8080)**

Access the petstore API (querying for a pet with ID 1) using the console on the right by using:

```shell
curl -s http://localhost:8080/api/pet/1 | jq '.'
```{{exec}}

This will show the pet details (something familiar making "furrr" usually...).
